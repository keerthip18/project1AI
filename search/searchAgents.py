        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if next_position in self.corners and next_position not in visited_corners:
                    next_visited_corners.append(next_position)

                successor_state = (next_position, tuple(next_visited_corners))
...
def cornersHeuristic(state, problem):
    ...
    min_distance = min(manhattan(position, corner) for corner in unvisited)
    ...
    if len(unvisited) > 1:
        ...
        current = position
        while remaining:
            nearest = min(remaining, key=lambda corner: manhattan(current, corner))
            total_dist += manhattan(current, nearest)
            current = nearest
...
def foodHeuristic(state, problem):
    ...
    # Maximum distance between any two food dots
    max_food_distance = 0
    ...
    return pacman_to_food + max_food_distance
